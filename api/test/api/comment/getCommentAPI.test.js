...
describe('Get comment endpoint API & integration tests', () => {
  const testuser = {
  ...
  test('successfully get comment, integration test', async () => {
    ...
    const response = await request(webapp).get(`/api/comments/${commentInDB._id}`).set('Authorization', `Bearer ${token}`);
    expect(response.body.userId).toBe(commentInDB.userId);
    expect(response.body.parentPostId).toBe(commentInDB.parentPostId);
    expect(response.body.content).toBe(commentInDB.content);
  });
});
