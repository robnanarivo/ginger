...
describe('update comment endpoint API & integration tests', () => {
  const testuser = {
  ...
  test('successfully update comment, integration test', async () => {
    ...
    const response = await request(webapp).put(`/api/comments/${commentInDB._id}`).set('Authorization', `Bearer ${token}`).send({
