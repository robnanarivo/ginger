...
describe('getPost Database operation tests', () => {
  // test data
  const tp = {
    creatorId: '123',
    groupId: '123',
  ...
  test('getPost, and integration test', async () => {
    ...
    await db.collection('groups').updateOne({ _id: ObjectId(testPost.groupId) }, { $push: { posts: postId.toString() } });
    const post = await postOperations.getPost(db, postId.toString());
